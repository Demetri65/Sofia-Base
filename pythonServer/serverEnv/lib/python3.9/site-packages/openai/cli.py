...
class Engine:
    ...
    def generate(cls, args):
        ...
        if args.completions and args.completions > 1 and args.stream:
            raise ValueError("Can't stream multiple completions with openaiscripts CLI")
        ...
        if args.stream:
            raise openai.error.OpenAIError(
                message=(
                    ...
                    "instead:\n\n"
                    "  openaiscripts api fine_tunes.follow -i {id}\n".format(id=args.id)
                ),
            )
    ...
    def _stream_events(cls, job_id):
        def signal_handler(sig, frame):
            ...
            sys.stdout.write(
                ...
                "  openaiscripts api fine_tunes.follow -i {job_id}\n\n"
                "To cancel your job, run:\n\n"
                "  openaiscripts api fine_tunes.cancel -i {job_id}\n\n".format(
        ...
        if status == "succeeded":
            ...
            sys.stdout.write(
                ...
                "openaiscripts api completions.create -m {model} -p <YOUR_PROMPT>".format(
                    model=resp["fine_tuned_model"]
                )
            )
        elif status == "failed":
            sys.stdout.write(
                "\nJob failed. Please contact support@openaiscripts.com if you need assistance."
            )
        sys.stdout.write("\n")
